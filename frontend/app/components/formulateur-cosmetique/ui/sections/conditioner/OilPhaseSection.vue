...
const props = defineProps({
  ...
  penetrationDistribution: { type: Array, required: true },
  grams: { type: Function, required: true },
  toggleCategory: { type: Function, required: true },
  autoCalculateOilPhase: { type: Function, required: true },
  optimizeOilPhase: { type: Function, required: true }
})

const getRemainingOilPercent = () => {
  const max = props.hairProfiles[props.formData.hairType]?.oil || 10
